    ...
    if (isSuccess) {
        return (
            <div className="flex flex-col gap-5 rounded-sm border border-border bg-surface p-8 md:p-10">
                ...
                <p className="text-sm text-muted leading-relaxed">{t("formSuccessBody")}</p>
                <button
                    type="button"
                    onClick={() => setIsSuccess(false)}
                    className="self-start text-xs font-medium tracking-[0.12em] uppercase text-foreground/60 underline-offset-4 hover:underline transition"
                >
                    {t("formSendAnother")}
                </button>
            </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div className="flex flex-col gap-2">
                    <Label htmlFor="name" required>{t("formNameLabel")}</Label>
                    <Input
                        id="name"
                        type="text"
                ...
                <div className="flex flex-col gap-2">
                    <Label htmlFor="email" required>{t("formEmailLabel")}</Label>
                    <Input
                        id="email"
                        type="email"
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div className="flex flex-col gap-2">
                    <Label htmlFor="subject" required>{t("formSubjectLabel")}</Label>
                    <Input
                        id="subject"
                        type="text"
            ...
            {/* Message */}
            <div className="flex flex-col gap-2">
                <Label htmlFor="message" required>{t("formMessageLabel")}</Label>
                <Textarea
                    id="message"
                    rows={6}
            ...
            <div className="flex items-center gap-4 pt-2">
                ...
                <span className="text-xs text-muted">
                    {tCommon("requiredFields").replace("*", "")}<span className="text-accent-warm">*</span>
                </span>
            </div>
        </form>
    );
